...
def _(
    ...
    sp, lp = eng.find_options_for_spread(
        SPEC,
        put_options,
        underlying_price,
        risk_free_rate,
...
@app.cell
def _(SPEC, eng, lp, sp, trade_client):
    res = eng.place_spread_order(SPEC, sp, lp, trade_client)
    return (res,)
...
@app.cell
def _(SPEC, eng, lp, mo, res, sp, symbol_select):
    if res is None:
        mo.stop(
            True,
            mo.callout(
    ...
    mo.vstack([
        ...
        eng.render_spread_ui(SPEC, lp, sp, symbol_select.value, order_result=res),
        eng.section_header(SPEC, "Payoff Chart", "3.2"),
        mo.ui.plotly(eng.payoff_chart(SPEC, lp, sp, symbol_select.value, **_payoff_kwargs)),
    ])
    return
