...
def _(HTTPAdapter, Retry, dt, io, pd, requests):
    ...
    FedInvest is a form-backed page, not a JSON API: it serves CSV when asked
    and HTML otherwise, and answers non-business days with a page containing no
    table rather than a 404. Both are handled below. If the form field names
    have drifted, _request() is the only function you need to touch.
    """
    ...
    def _request(day):
        """POST the date form. Returns the raw body (CSV if we got it, else HTML)."""
        payload = {
            "priceDate.month": day.month,
            "priceDate.day": day.day,
            "priceDate.year": day.year,
            "submit": "Show Prices",
            "csv": "CSV FORMAT",
        }
...
def _(ThreadPoolExecutor, pathlib, pd, session):
    _TERMS_CACHE = pathlib.Path(".tdterms.parquet")
    ...
    def fetch_terms(cusips):
        ...
        auction (earliest issue date); `outstanding` and `n_auctions` from
        the latest, because reopenings change them.
        ...
        try:
            cached = (pd.read_parquet(_TERMS_CACHE) if _TERMS_CACHE.exists()
                      else pd.DataFrame(columns=_cols))
        except Exception:                       # partially written / corrupt file
            cached = pd.DataFrame(columns=_cols)
        if list(cached.columns) != _cols:       # cache from an older schema
            cached = pd.DataFrame(columns=_cols)
        ...
        def one(c):
            try:
                ...
                failed.append(c)                # stale row stands in, if any
                return None
            if not recs:
                return None                     # unknown CUSIP: legitimately empty
        ...
        if new:
            tmp = _TERMS_CACHE.with_suffix(".tmp")  # atomic swap: a crash
            have.to_parquet(tmp)                    # mid-write can't corrupt
            tmp.replace(_TERMS_CACHE)               # the live cache
        return have, failed
...
def _(fetch_latest, fetch_mspd, fetch_terms, mo):
    ...
    prices = prices.merge(_outstanding_mspd, on="cusip", how="left",
                          validate="m:1")
    prices["outstanding"] = prices["outstanding_mspd"].fillna(prices["outstanding"])
    prices = prices.drop(columns=["outstanding_mspd"])
...
def _(dt, mo, np, pd, prices, rl):
    ...
    _settle = rl.add_tenor(_all_bills["price_date"].iloc[0].to_pydatetime(),
                           "1b", "F", "nyc")
    ...
    _maxd = int(_d.max())
    _node_days = ([0] + [t for t in (7, 14, 21, 30, 45, 60, 91, 121, 182, 273, 364)
                         if t < _maxd] + [_maxd])
    _curve = rl.Curve(
    ...
    # Implied forward path: Curve.rate over consecutive 4-week windows — the
    # market's expected short rate, computed by rateslib rather than by hand.
    ...
    def _rows(_df, _col, _unit):
        ...
        return (f'<div class="fi-scroll"><table class="fi-tbl"><thead><tr><th>CUSIP</th><th>Matures</th>'
                ...
                f'<th class="fi-r">{_unit}</th><th class="fi-r">Spread</th>'
    ...
    _excl = (f' <span style="color:{_SIENNA}">Excluded: '
             f'{", ".join(_dropped)}</span> — quoted on one side only, or inside '
             f'seven days of settlement.' if _dropped else "")
    ...
    <div class="fi">
      ...
      <div class="fi-slope">Front to one year
        ...
        the market prices easing into {_trough["maturity_date"]:%B}</div>
      ...
      <div class="fi-grid">
        <div><div class="fi-h">Cheapest to the curve</div>
          <div class="fi-note">Yield above fair. Worth acting on only when the
            edge clears the spread.</div>{_rows(_cheap, "resid_bp", "Edge bp")}</div>
        <div><div class="fi-h">Best four-week carry</div>
          <div class="fi-note">P&amp;L in bp of face against financing at
            {repo_rate:.3f}%, curve unchanged.</div>
...
def _(FI_CSS, bonds, dt, mo, np, pd, prices, rl):
    ...
    _u["spread_bp"] = np.where((_u["buy"] > 0) & (_u["sell"] > 0),
    ...
    _u = _u[(_u["mid"] > 0) & (_u["days"] > 90)].sort_values("days")
    ...
    <div class="fi">
      ...
      <div class="fi-slope">Missing the strip bid costs them
        <b style="color:{_t2(_gap)}">{_gap:+.1f} bp</b> of yield ·
        the spread to trade them is {legacy_bonds["spread_bp"].median():.0f} bp</div>
      <div class="fi-scroll"><table class="fi-tbl"><thead><tr><th>CUSIP</th><th class="fi-r">Coupon</th>
        ...
        <th class="fi-r">Dur</th><th class="fi-r">Spread</th></tr></thead>
      ...
      <div class="fi-note" style="margin-top:14px">Edge is yield against the
        ...
        {_sd:.1f}bp. Every edge here is smaller than the spread to cross.</div>
    </div>""")
    return (ust_curve,)
...
def _(FI_CSS, dt, mo, np, os, pd, prices, rl, session, tips, ust_curve):
    ...
    if _has_cpi:
        _t["ratio"] = [float(_o.index_ratio(_settle)) for _o in _obj]
    ...
    _keep = _t["two_sided"].values
    _rk = sorted({int(_y * 365.25) for _y in [1, 2, 3, 4, 5, 7, 10, 12, 15, 20, 25, 30]
                  if _y * 365.25 <= _t["days"].max()} | {0, int(_t["days"].max())})
    ...
    _t["edge_bp"] = np.where(_t["days"] > 365,
    ...
    if _has_cpi:
        _lock = (float(_obj[-1].index_ratio(_known))
                 / float(_obj[-1].index_ratio(_settle)) - 1)
    ...
    <div class="fi">
      ...
      <div class="fi-slope">Missing the strip bid is worth
        <b style="color:{_tn3(_gap)}">{_gap:+.1f} bp</b> of real yield ·
        the spread to trade them is {five_tips["spread_bp"].median():.1f} bp, so
        the edge clears the cost</div>
      {_lockline}
      <div class="fi-panel">{_chart}
      ...
      <div class="fi-scroll"><table class="fi-tbl"><thead><tr><th>CUSIP</th><th class="fi-r">Coupon</th>
        ...
        <th class="fi-r">Spread</th></tr></thead>
...
def _(FI_CSS, dt, mo, np, pd, prices, rl, ust_curve):
    ...
    # Eight two-year FRNs, every one indexed to the SAME thing: the high
    # discount rate of the most recent 13-week bill auction, reset weekly,
    # paid quarterly, plus a spread fixed at the note's own auction. So the
    # only thing separating them is that spread and what you pay for it —
    # which is exactly what a discount margin measures.
    _settle = rl.add_tenor(prices["price_date"].iloc[0].to_pydatetime(),
                           "1b", "F", "nyc")
    _f = prices[prices["security_type"] == "MARKET BASED FRN"].copy()
    _f = _f[_f["mid"] > 0].sort_values("maturity_date")
    ...
    # FedInvest carries the CURRENT coupon; the spread is fixed for life, so
    # the difference is today's index. It must come out the same for all
    # eight — that is the check that the arithmetic is the right way round.
    _f["index"] = _f["rate"] - _f["frn_spread"]

    # rateslib refuses a rate curve whose convention disagrees with the float
    # leg's accrual, and rightly: ust_curve is Act365F, a Treasury FRN accrues
    # Act360. Rebuild it on its OWN discount factors under Act360 — nodes are
    # DFs, so discounting is bit-identical and only the rate conventions
    # change. Used for forecasting and discounting both, which also makes the
    # resulting margin an Act360 number, as the market quotes it.
    _a360 = rl.Curve(
        nodes={_d: float(_v) for _d, _v in ust_curve.nodes.nodes.items()},
    ...
    _f["accrued"] = [float(_n.accrued(_settle, rate_curve=_a360))
                     for _n in _notes]
    # Discount margin from rateslib's own oaspread — "the spread added to the
    # discounting curve to value the security at price", which for a note
    # whose index is forecast off that same curve is precisely the margin.
    _f["dm_bp"] = [float(_n.oaspread(price=float(_r["mid"]), curves=_a360))
                   for _n, (_, _r) in zip(_notes, _f.iterrows())]
    # A note days from redemption prices off its redemption, not its margin:
    # two hundredths of a point over four days annualises to nonsense.
    _use = (_f["two_sided"] & (_f["days"] >= 30)).values
    _f["dm_bp"] = np.where(_use, _f["dm_bp"], np.nan)

    # The margin curve is a straight line across eight points; the residual
    # from it is the only relative value on offer.
    _fit = np.polyfit(_f.loc[_use, "yrs"], _f.loc[_use, "dm_bp"], 1)
    _f["fit_bp"] = np.polyval(_fit, _f["yrs"])
    _f["edge_bp"] = np.where(_use, _f["dm_bp"] - _f["fit_bp"], np.nan)
    _sd = float(np.nanstd(_f["edge_bp"], ddof=1))

    # The stated bias. The index is a DISCOUNT rate; the curve forecasts
    # money-market yields, which on the same bill sit higher by this much.
    # rateslib's own Bill converts between the two rather than us asserting a
    # formula. It moves every margin the same way, so the ranking survives.
    _idx = float(_f["index"].median())
    _b91 = rl.Bill(effective=_settle,
                   termination=_settle + dt.timedelta(days=91), spec="us_gbb")
    _px91 = 100 * (1 - _idx / 100 * 91 / 360)
    _bias = (float(_b91.simple_rate(price=_px91, settlement=_settle))
             - float(_b91.discount_rate(price=_px91, settlement=_settle))) * 100
    ...
    # Signature: quoted margin against the margin actually kept. Gold is what
    # the coupon promises, green what survives paying above par; the drop
    # between them is the premium, note by note.
    _W, _CH, _PADL, _PADR, _CT, _CB = 1000, 300, 64, 24, 26, 44
    _lo = min(float(np.nanmin(_f["dm_bp"])), 0.0) - 3
    _hi = float(_f["frn_spread"].max()) * 100 + 3
    _maxy = float(_f["yrs"].max())
    _xs = lambda _y: _PADL + (_y / _maxy) * (_W - _PADL - _PADR)
    _ys = lambda _v: _CT + (_hi - _v) / (_hi - _lo) * (_CH - _CT - _CB)

    _rules, _tick = [], float(np.ceil(_lo / 5) * 5)
    while _tick <= _hi:
        _gy = _ys(_tick)
        _rules.append(
            f'<line x1="{_PADL}" y1="{_gy:.1f}" x2="{_W - _PADR}" y2="{_gy:.1f}" '
    ...
    _line = " ".join(f"{_xs(_y):.1f},{_ys(_v):.1f}" for _y, _v
                     in zip(_f.loc[_use, "yrs"], _f.loc[_use, "fit_bp"]))
    _pairs = "".join(
        f'<line x1="{_xs(_r["yrs"]):.1f}" y1="{_ys(_r["frn_spread"] * 100):.1f}" '
        f'x2="{_xs(_r["yrs"]):.1f}" y2="{_ys(_r["dm_bp"]):.1f}" '
        f'stroke="{_SLATE}" stroke-width="1" opacity="0.45"/>'
        f'<circle cx="{_xs(_r["yrs"]):.1f}" cy="{_ys(_r["frn_spread"] * 100):.1f}" '
        f'r="2.6" fill="{_GOLD}"/>'
        f'<circle cx="{_xs(_r["yrs"]):.1f}" cy="{_ys(_r["dm_bp"]):.1f}" r="3.4" '
        f'fill="{_tf(_r["edge_bp"])}" stroke="#0C110F" stroke-width="1">'
        f'<title>{_r["cusip"]} · {_r["maturity_date"]:%b %Y} · quoted '
        f'{_r["frn_spread"] * 100:.1f}bp · kept {_r["dm_bp"]:.2f}bp · off the '
        f'line {_r["edge_bp"]:+.2f}bp</title></circle>'
        for _, _r in _f[_use].iterrows())
    _axis = "".join(
        f'<text x="{_xs(_y):.1f}" y="{_CH - 14}" fill="{_SLATE}" '
        f'text-anchor="middle" font-family="DM Mono, monospace" font-size="10">'
        f'{_y:g}y</text>' for _y in (0.5, 1.0, 1.5) if _y <= _maxy)
    _chart = (
        f'<svg viewBox="0 0 {_W} {_CH}" width="100%" role="img" '
        f'aria-label="Quoted margin against realised discount margin by maturity">'
        + "".join(_rules)
        + f'<polyline points="{_line}" fill="none" stroke="{_GREEN}" '
          f'stroke-width="1.5" opacity="0.55"/>' + _pairs + _axis + '</svg>')

    _rows = "".join(
        f'<tr><td class="fi-cusip">{_r["cusip"]}</td>'
        f'<td>{_r["maturity_date"]:%b %Y}</td>'
        f'<td class="fi-r">{_r["rate"]:.3f}</td>'
        f'<td class="fi-r" style="color:{_GOLD}">{_r["frn_spread"] * 100:.1f}</td>'
        f'<td class="fi-r">{_r["mid"]:.4f}</td>'
        f'<td class="fi-r fi-dim">{_r["accrued"]:.3f}</td>'
        f'<td class="fi-r">{_cell(_r["dm_bp"], "+.2f")}</td>'
        f'<td class="fi-r" style="color:{_tf(_r["edge_bp"])}">'
        f'{_cell(_r["edge_bp"], "+.2f")}</td>'
        f'<td class="fi-r fi-dim">{_cell(_r["spread_bp"], ".2f")}</td></tr>'
        for _, _r in _f.iterrows())

    _off = sorted(_f.loc[~_use, "cusip"])
    _excl = (f' <span style="color:{_SIENNA}">{", ".join(_off)}</span> sits inside '
             f'thirty days of redemption, where price says nothing about margin.'
             if _off else "")
    frn_desk = _f
    ...
    <div class="fi">
      ...
      <div class="fi-deck">All {len(_f)} are two-year notes on the same index, so
        the only question is the spread and what you pay for it. Every one
        trades above par, which means the margin you keep is less than the
        margin printed on the note.{_excl}</div>
      <div class="fi-stats">
        <div class="fi-stat"><div class="fi-lbl">Index today</div>
          <div class="fi-num">{_hd(_idx)[0]}<span>{_hd(_idx)[1]}</span><em>%</em></div>
          <div class="fi-sub">identical across all {len(_f)}</div></div>
        <div class="fi-stat"><div class="fi-lbl">Margin curve</div>
          <div class="fi-num">{_fit[0]:+.1f}<em>bp/yr</em></div>
          <div class="fi-sub">{np.nanmin(_f["dm_bp"]):+.1f} to {np.nanmax(_f["dm_bp"]):+.1f} bp</div></div>
        <div class="fi-stat"><div class="fi-lbl">Widest edge</div>
          <div class="fi-num" style="color:{_SLATE}">{np.nanmax(np.abs(_f["edge_bp"])):.2f}<em>bp</em></div>
          <div class="fi-sub">against a {_sd:.2f}bp line</div></div>
        <div class="fi-stat"><div class="fi-lbl">Bid-ask</div>
          <div class="fi-num">{_f["spread_bp"].median():.2f}<em>bp</em></div>
          <div class="fi-sub">wider than every edge</div></div>
      </div>
      <div class="fi-slope">The sector prices as one line — every note sits
        within <b>{np.nanmax(np.abs(_f["edge_bp"])):.2f} bp</b> of it, and the
        spread to cross is <b>{_f["spread_bp"].median():.2f} bp</b> · there is no
        trade here today</div>
      <div class="fi-panel">{_chart}
        <div class="fi-cap"><span class="fi-k fi-k-dot" style="background:{_GOLD}"></span>Quoted margin
          <span class="fi-k fi-k-dot"></span>Margin kept
          <span class="fi-k fi-k-line"></span>Fitted · the drop between the pair is the premium paid</div></div>
      <div class="fi-scroll"><table class="fi-tbl"><thead><tr><th>CUSIP</th><th>Matures</th>
        <th class="fi-r">Coupon</th><th class="fi-r">Quoted</th>
        <th class="fi-r">Price</th><th class="fi-r">Acc</th>
        <th class="fi-r">Margin</th><th class="fi-r">Edge</th>
        <th class="fi-r">Spread</th></tr></thead>
        <tbody>{_rows}</tbody></table></div>
      <div class="fi-note" style="margin-top:14px">Margin is rateslib's
        <code>oaspread</code> — the spread over the projected index that
        reprices the note. Levels carry a known bias of about
        <b>{_bias:.1f} bp</b>: the index is a discount rate, while the curve
        forecasts money-market yields, which on the same 13-week bill sit that
        much higher. It moves all {len(_f)} the same way, so the edge column
        and the ranking hold; only the absolute level is affected.</div>
    </div>""")
    return (frn_desk,)
...
def _(FI_CSS, mo, np, pd, prices, rl, ust_curve):
    ...
    _live = _nom & (_s["mid"] > 0) & (_s["days"] > 90)
    ...
    _tot = _s["stripped"].sum()
    _recon = _s["reconstituted"].sum()
    ...
    _yv = _lg["yrs"].values
    _presid = _pr_ - np.polyval(np.polyfit(_yv, _pr_, 2), _yv)
    _eresid = _er - np.polyval(np.polyfit(_yv, _er, 2), _yv)
    _rho = float(np.corrcoef(_presid, _eresid)[0, 1])
    _lg["q"] = pd.qcut(_lg["pct"], 4, labels=["Least", "Q2", "Q3", "Most"])
    _qt = _lg.groupby("q", observed=True).agg(
        n=("cusip", "size"), pct=("pct", "median"), edge=("edge_bp", "median"),
        yrs=("yrs", "median")).reset_index()
    ...
    _agg = _yr.groupby("y").agg(bn=("stripped", lambda _v: _v.sum() / 1e9),
                                cs=("corpus_share", "median")).reset_index()
    _agg = _agg[_agg["y"] <= int(_s["maturity_date"].dt.year.max())]
    _W, _CH, _PADL, _PADR, _CT, _CB = 1000, 320, 58, 46, 24, 42
    _y0, _y1 = int(_agg["y"].min()), int(_agg["y"].max())
    _bmax = float(_agg["bn"].max())
    ...
    _rows = "".join(
        ...
        f'<td class="fi-r">{_r["rate"]:.3f}</td>'
        f'<td>{_r["maturity_date"]:%b %Y}</td>'
        f'<td class="fi-r">${_r["stripped"] / 1e9:.1f}</td>'
        f'<td class="fi-r" style="color:{_GOLD}">{_r["pct"] * 100:.1f}%</td>'
        f'<td class="fi-r fi-dim">{_r["corpus_share"] * 100:.0f}%</td>'
        f'<td class="fi-r">{_r["mod_dur"]:.1f}</td>'
        f'<td class="fi-r" style="color:{_GREEN if _r["edge_bp"] > 0 else _SIENNA}">'
        f'{_r["edge_bp"]:+.2f}</td>'
        f'<td class="fi-r fi-dim">${_r["reconstituted"] / 1e6:,.0f}m</td></tr>'
        for _, _r in _top.iterrows())

    _note = (f' <span style="color:{_SIENNA}">{", ".join(_miss)}</span> has no '
             f'Table 5 row yet — too recently issued for a corpus CUSIP.'
             if _miss else "")
    strips_desk = _s
    ...
    <div class="fi">
      ...
      <div class="fi-stats">
        ...
        <div class="fi-stat"><div class="fi-lbl">TIPS stripped</div>
          ...
          <div class="fi-sub">strippable, never stripped</div></div>
      </div>
      <div class="fi-panel">{_chart}
      ...
      <div class="fi-grid">
        <div><div class="fi-h">The strip bid does not make a bond rich</div>
          <div class="fi-note">Bonds past twenty years, sorted into quartiles by
            ...
            heavily stripped quartile trades <b>{_gap:.1f}bp cheaper</b> than the
            least, not richer — and the relationship holds at
            <b>{_rho:+.2f}</b> after taking a quadratic in tenor out of both
            sides, so it is not tenor in disguise. The readable direction is
            that stripping follows cheapness rather than causing richness: you
            strip the bond that is cheap to buy. At long-bond spreads this is
            an observation, not a trade.</div>
        ...
        <div><div class="fi-h">What stripping manufactures</div>
          ...
          <div class="fi-note">Reconstitution runs at
            <b>${_recon / 1e9:.1f}bn</b> a month against
            <b>${_tot / 1e9:,.0f}bn</b> outstanding — the pieces move back and
