...
def _(mo):
    ...
    <div style="padding: 2.5rem 0 2rem; text-align: center;">
      ...
      <h1 style="font-family: 'DM Serif Display', serif; font-size: 36px; font-weight: 400; font-style: italic; margin: 0 0 6px; letter-spacing: -0.01em; color: var(--color-text-primary);">Bull Call Spread Trading Strategy</h1>
...
def _(
    ...
    criteria = {
        'short_call': ((21, 90), (realized_vol * 0.8, realized_vol * 2.5), (0.20, 0.45), (0.05, 0.60)),
        'long_call':  ((21, 90), (realized_vol * 0.8, realized_vol * 2.5), (0.45, 0.80), (0.05, 0.60)),
    }
...
def _(logger):
    def pair_call_candidates(short_calls, long_calls, underlying_price):
        """
        Build the BEST bull call spread from the candidate legs.

        A bull call spread only requires long_strike < short_strike; moneyness is
        already governed by the delta bands in `criteria`. (The old code also
        demanded the spot sit between the strikes, which silently excluded every
        standard OTM/OTM construction.)

        Every valid pair is scored on reward:risk = (width - debit) / debit and
        the best one is returned. Previously the FIRST pair encountered won,
        which made the choice depend on arbitrary API ordering.
        """
        best, best_score = None, None

        for sc in short_calls:
            for lc in long_calls:
                if sc['expiration_date'] != lc['expiration_date']:
                    continue
                if sc['symbol'] == lc['symbol']:
                    continue
                if not (lc['strike_price'] < sc['strike_price']):
                    continue

                debit = lc['initial_option_price'] - sc['initial_option_price']
                ...
                if best_score is None or score > best_score:
                    best, best_score = (sc, lc), score

        if best is not None:
            sc, lc = best
            logger.info(
                f"Selected Bull call spread: short_call {sc['symbol']} and long_call "
                f"{lc['symbol']} with expiration {sc['expiration_date']} "
...
def _(Any, Dict, logger):
    def check_buying_power(short_call: Dict[str, Any], long_call: Dict[str, Any], buying_power_limit: float) -> None:
        ...
        debit = long_call['initial_option_price'] - short_call['initial_option_price']

        # A bull call spread MUST be a net debit (long leg is the lower strike and
        # therefore costs more). abs() used to hide an inverted/mispriced pair by
        # turning a credit into a plausible-looking risk number.
        if debit <= 0:
            raise Exception(
                f'Not a debit spread: long {long_call["symbol"]} @ '
                f'{long_call["initial_option_price"]} is not priced above short '
                f'{short_call["symbol"]} @ {short_call["initial_option_price"]}.'
            )
...
def _(
    ...
    def find_options_for_bull_call_spread(call_options, underlying_price, risk_free_rate, buying_power_limit, criteria, OI_THRESHOLD):
        ...
        for option_data in call_options:
            ...
            if candidate is None:
                ...
                # expiry); the previous `return` aborted the entire scan on the
                # first such contract, so a spread was almost never found.
                logger.debug(
        ...
        # SORTED (nearest expiry first) — iterating a raw set gave an arbitrary,
        # run-to-run-unstable choice of expiration.
        common_expirations = sorted(
            set(short_call_candidates_by_exp) & set(long_call_candidates_by_exp)
        )
    ...
    return (find_options_for_bull_call_spread,)
...
def _(
    ...
    buying_power_limit,
    criteria,
    find_options_for_bull_call_spread,
    get_options,
    max_expiration,
    max_strike,
    min_expiration,
...
def _(
    ...
    def place_bull_call_spread_order(short_call, long_call):
        ...
        try:
            ...
            net_debit = (long_call['initial_option_price'] - short_call['initial_option_price'])
    ...
    return (place_bull_call_spread_order,)


@app.cell
def _(lc, place_bull_call_spread_order, sc):
...
def _(go, mo, np, symbol_select):
    ...
    def render_bull_call_spread_ui(
        long_call: dict,
        short_call: dict,
        order_result=None,
        error: str = None,
        ...
        """
        Render a styled Marimo UI card for a bull call spread order.

        Args:
            short_call:    dict with 'symbol' and 'initial_option_price'
            long_call:     dict with 'symbol' and 'initial_option_price'
            order_result:  Alpaca Order object returned by trade_client.submit_order()
            error:         error string from the except block (or None)
        """
        if not (short_call and long_call):
            return mo.callout(
                mo.md("**No valid bull call spread found.** Provide both `short_call` and `long_call`."),
                kind="warn",
            )

        # lower strike call = long call
        # higher strike call = short call

        long_price  = long_call["initial_option_price"]
        short_price = short_call["initial_option_price"]

        net_debit = round(long_price - short_price, 2)

        long_strike  = _parse_strike(long_call["symbol"])
        short_strike = _parse_strike(short_call["symbol"])

        spread_width = (
            round(short_strike - long_strike, 2)
        ...
        def leg_card(leg_data, input_call, badge_bg, badge_color, arrow, action_label):
            """Render one leg card, merging live order data with input prices."""
            symbol       = leg_data["symbol"]   if leg_data else input_call["symbol"]
            ...
            <div style="background:var(--color-background-secondary);border-radius:8px;padding:14px 16px;">
              ...
              <p style="font-size:12px;color:var(--color-text-secondary);margin:0 0 2px">
                Premium: <strong>${input_call['initial_option_price']:.2f}</strong>
              </p>
        ...
        <div style="font-family:'DM Mono',monospace;padding:1.5rem 0">
          ...
          <p style="font-family:'DM Serif Display',serif;font-size:22px;
                    color:var(--color-text-primary);margin:0 0 4px">Bull Call Spread</p>
          ...
          <div style="background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:1rem">
              {leg_card(sell_leg, short_call, "#FAECE7", "#993C1D", "↓", "Sell to open")}
              {leg_card(buy_leg,  long_call,  "#E1F5EE", "#0F6E56", "↑", "Buy to open")}
            </div>
    ...
    def bull_call_payoff_chart(long_call, short_call):

        long_price  = long_call["initial_option_price"]
        short_price = short_call["initial_option_price"]

        K_long  = _parse_strike(long_call["symbol"])
        K_short = _parse_strike(short_call["symbol"])

        debit = long_price - short_price

        breakeven = K_long + debit
        max_loss = debit * 100
        max_gain = (K_short - K_long - debit) * 100

        S = np.linspace(K_long * 0.8, K_short * 1.25, 500)
    ...
    return bull_call_payoff_chart, render_bull_call_spread_ui


@app.cell
def _(
    bull_call_payoff_chart,
    lc,
    mo,
    render_bull_call_spread_ui,
    res,
    sc,
    section_header,
):
    if res is None:
        mo.stop(
            True,
            mo.callout(
                mo.md("**No valid bull call spread found.**"),
                kind="warn"
            )
        )
    ...
    mo.vstack([
        ...
        render_bull_call_spread_ui(lc, sc, order_result=res),
